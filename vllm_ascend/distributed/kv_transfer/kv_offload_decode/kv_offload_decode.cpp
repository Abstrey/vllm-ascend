...
int warmup_lru_resident_threads(const int64_t requested_threads) {
    ...
    TORCH_CHECK(
        observed_threads == active_threads,
        "OpenMP planner warmup created an unexpected thread count: expected=",
        active_threads,
        ", actual=",
        observed_threads);
...
FORCE_INLINE void process_one_lru_resident_row(
    ...
    int16_t* RESTRICT encoded_plan,
    const int32_t encoded_plan_stride,
    const bool encode_physical_row,
    const int32_t* RESTRICT visible_seq_lens
) {
    ...
    int16_t* RESTRICT encoded_plan_row = encoded_plan == nullptr
        ? nullptr
        : encoded_plan + static_cast<int64_t>(logical_row) * encoded_plan_stride;
    const int32_t resident_capacity = current_token_slots == nullptr
        ? capacity
        : capacity - 1;
    ...
    const int32_t stable_prefix_len = std::clamp(
        stable_prefix_lens[logical_row],
        0,
        max_token
    );
    const int32_t visible_seq_len = visible_seq_lens == nullptr
        ? max_token
        : std::clamp(visible_seq_lens[logical_row], 0, max_token);
    ...
    for (int32_t pos = 0; pos < topk; ++pos) {
        ...
        if (LIKELY(is_valid_lru_resident_token(token, max_token)) &&
            token < visible_seq_len && token_mark[token] != base) {
    ...
    for (int32_t pos = 0; pos < topk; ++pos) {
        const int32_t token = topk_row[pos];
        if (LIKELY(is_valid_lru_resident_token(token, max_token)) &&
            token < visible_seq_len && current_slots_row[pos] < 0) {
    ...
    for (int32_t miss_idx = 0; miss_idx < assign_count; ++miss_idx) {
        ...
        if (encoded_plan_row != nullptr) {
            const bool is_current_token =
                visible_seq_lens != nullptr && token == visible_seq_len - 1;
            encoded_plan_row[pos] = static_cast<int16_t>(
                is_current_token ? slot + 1 : -(slot + 1));
        }
    ...
    if (current_token_slots != nullptr) {
        current_token_slots[logical_row] =
            physical_row * capacity + current_token_slot;
    }
}
...
HOT_FUNCTION void lru_resident_compact_impl(
    ...
    auto* RESTRICT physical_row_workspace =
        reinterpret_cast<int32_t*>(physical_row_workspace_ptr);
    ...
    int32_t* RESTRICT physical_row_used = physical_row_workspace == nullptr
        ? nullptr
        : physical_row_workspace + physical_row_capacity_int;
    int32_t* RESTRICT current_token_slots = physical_row_workspace == nullptr
        ? nullptr
        : physical_row_workspace + static_cast<int64_t>(physical_row_capacity_int) * 2;
    const int32_t* RESTRICT visible_seq_lens =
        reinterpret_cast<int32_t*>(visible_seq_lens_ptr);
    if (logical_to_physical == nullptr) {
        ...
        for (int logical_row = 0; logical_row < num_reqs_int; ++logical_row) {
            for (int physical_row = 0; physical_row < physical_row_capacity_int; ++physical_row) {
                if (physical_row_used[physical_row] == 0 &&
                    last_req_ids[physical_row] == req_ids[logical_row]) {
    ...
    if (active_threads == 1) {
        for (int row = 0; row < num_reqs_int; ++row) {
            const int physical_row = logical_to_physical == nullptr
                ? row
                : logical_to_physical[row];
            process_one_lru_resident_row(
                ...
                current_token_slots,
                encoded_plan,
                static_cast<int32_t>(encoded_plan_stride),
...
HOT_FUNCTION void lru_resident_compact(
    ...
    lru_resident_compact_impl(
        req_ids_ptr, last_req_ids_ptr, topk_indices_ptr,
        stable_prefix_lens_ptr, slot_to_token_ptr, lru_slots_ptr,
        current_slots_ptr, miss_count_ptr, miss_tokens_ptr,
        miss_slots_ptr, token_mark_workspace_ptr, token_pos_workspace_ptr,
        slot_workspace_ptr, miss_position_workspace_ptr, epochs_ptr,
        num_reqs, topk, capacity, max_token, workspace_threads,
        requested_threads, 0, 0, 0, num_reqs, 0);
}
...
HOT_FUNCTION void lru_resident_compact_with_plan_stable_rows(
    ...
    TORCH_CHECK(physical_row_workspace_ptr != 0,
                "stable-row planner requires physical_row_workspace_ptr");
    TORCH_CHECK(physical_row_capacity >= num_reqs,
                "stable-row planner capacity must cover all logical rows");
    TORCH_CHECK(physical_row_capacity <= std::numeric_limits<int16_t>::max(),
                "stable-row planner capacity exceeds int16 row encoding");
    TORCH_CHECK(encoded_plan_ptr != 0,
                "stable-row planner requires encoded_plan_ptr");
    TORCH_CHECK(encoded_plan_stride >= topk + 7,
                "stable-row encoded_plan_stride must include control row");
    TORCH_CHECK(capacity <= std::numeric_limits<int16_t>::max(),
                "resident capacity exceeds int16 plan encoding");
    TORCH_CHECK(visible_seq_lens_ptr != 0,
                "stable-row planner requires visible_seq_lens_ptr");
    lru_resident_compact_impl(
        req_ids_ptr, last_req_ids_ptr, topk_indices_ptr,
        stable_prefix_lens_ptr, slot_to_token_ptr, lru_slots_ptr,
        current_slots_ptr, miss_count_ptr, miss_tokens_ptr,
        ...
        slot_workspace_ptr, miss_position_workspace_ptr, epochs_ptr,
        num_reqs, topk, capacity, max_token, workspace_threads,
        requested_threads, encoded_plan_ptr, encoded_plan_stride,
        physical_row_workspace_ptr, physical_row_capacity,
...
{
    static auto* registries = new std::unordered_map<
        aclmdlRI, std::unique_ptr<LruResidentCompactGraphPayloadRegistry>>();
    return *registries;
}
...
void delete_lru_resident_compact_graph_payloads(void* args) noexcept
{
    ...
    std::unique_ptr<LruResidentCompactGraphPayloadRegistry> owned_registry;
    {
        std::lock_guard<std::mutex> lock(
            lru_resident_compact_graph_registry_mutex());
...
LruResidentCompactWithPlanPayload* retain_lru_resident_compact_graph_payload(
    aclmdlRI model_ri,
    std::unique_ptr<LruResidentCompactWithPlanPayload> payload)
{
    ...
    if (it == registries.end()) {
        ...
        const aclError register_ret = aclmdlRIDestroyRegisterCallback(
            model_ri,
            delete_lru_resident_compact_graph_payloads,
            raw_registry);
        if (register_ret != ACL_SUCCESS) {
            registries.erase(it);
        }
        TORCH_CHECK(register_ret == ACL_SUCCESS,
                    "failed to bind stable-row external LRU payload registry to graph lifetime, error code: ",
                    register_ret);
    }
...
void lru_resident_compact_with_plan_callback(void* args) noexcept
{
    ...
    lru_resident_compact_impl(
        payload->req_ids_ptr,
        payload->last_req_ids_ptr,
        payload->topk_indices_ptr,
        payload->stable_prefix_lens_ptr,
        payload->slot_to_token_ptr,
        payload->lru_slots_ptr,
        payload->current_slots_ptr,
        payload->miss_count_ptr,
        payload->miss_tokens_ptr,
        payload->miss_slots_ptr,
        payload->token_mark_workspace_ptr,
        payload->token_pos_workspace_ptr,
        payload->slot_workspace_ptr,
        payload->miss_position_workspace_ptr,
        payload->epochs_ptr,
        payload->num_reqs,
        payload->topk,
        payload->capacity,
        payload->max_token,
        payload->workspace_threads,
        payload->requested_threads,
        payload->encoded_plan_ptr,
        payload->encoded_plan_stride,
        payload->physical_row_workspace_ptr,
        payload->physical_row_capacity,
        payload->visible_seq_lens_ptr);
    ...
    uintptr_t visible_seq_lens_ptr)
{
    ...
    const aclError capture_ret = aclmdlRICaptureGetInfo(
        stream, &capture_status, &model_ri);
    TORCH_CHECK(capture_ret == ACL_SUCCESS,
                "aclmdlRICaptureGetInfo for stable-row external LRU plan failed, error code: ",
                capture_ret);
    ...
    const bool graph_lifetime =
        capture_status == ACL_MODEL_RI_CAPTURE_STATUS_ACTIVE;
    TORCH_CHECK(!graph_lifetime || model_ri != nullptr,
                "active graph capture returned a null model runtime instance");

    auto payload = std::make_unique<LruResidentCompactWithPlanPayload>(
        LruResidentCompactWithPlanPayload{
            req_ids_ptr, last_req_ids_ptr, topk_indices_ptr,
            stable_prefix_lens_ptr, slot_to_token_ptr, lru_slots_ptr,
            current_slots_ptr, miss_count_ptr, miss_tokens_ptr,
            miss_slots_ptr, token_mark_workspace_ptr, token_pos_workspace_ptr,
            slot_workspace_ptr, miss_position_workspace_ptr, epochs_ptr,
            encoded_plan_ptr, encoded_plan_stride,
            physical_row_workspace_ptr, physical_row_capacity,
            visible_seq_lens_ptr,
            num_reqs, topk, capacity, max_token, workspace_threads,
            requested_threads, !graph_lifetime});
    auto* raw_payload = payload.get();
    if (graph_lifetime) {
        raw_payload = retain_lru_resident_compact_graph_payload(
            model_ri, std::move(payload));
    }
    const aclError ret = aclrtLaunchHostFunc(
        stream,
        lru_resident_compact_with_plan_callback,
        raw_payload);
    if (ret == ACL_SUCCESS && !graph_lifetime) {
        payload.release();
    }
    TORCH_CHECK(ret == ACL_SUCCESS,
                "aclrtLaunchHostFunc for stable-row external LRU plan failed, error code: ",
                ret);
}
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m)
{
    ...
    m.def("lru_resident_compact_with_plan_stable_rows",
          &lru_resident_compact_with_plan_stable_rows,
          "CPU LRU resident compact with encoded plan and stable physical rows");
    m.def("enqueue_lru_resident_compact_with_plan_stable_rows",
          &enqueue_lru_resident_compact_with_plan_stable_rows,
