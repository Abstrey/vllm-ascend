...
def test_mtp_rewrite_invalidates_membership_slot_map():
    ...
    control = membership[
        :, FSA_SELECTION_MEMBERSHIP_CONTROL_OFFSET_INT16_COUNT :
        FSA_SELECTION_MEMBERSHIP_CONTROL_OFFSET_INT16_COUNT + 8
    ]
    control[:, 0] = 0x5A4D
...
def test_fused_overlap_external_plan_passes_raw_topk_and_full_selection_state():
    ...
    def allocate_membership(row_capacity):
        ...
        control[:, 2] = 4
        control[:, 3] = FSA_SELECTION_MEMBERSHIP_CONTROL_OFFSET_INT16_COUNT - 4
        return membership
    ...
    with (
        ...
        patch(
            "vllm_ascend.attention.sfa_kv_offload.get_kv_offload_decode_manager",
