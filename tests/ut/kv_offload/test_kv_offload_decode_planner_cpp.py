...
def planner_helper():
    ...
    return cpp_extension.load(
        name="kv_offload_decode_planner_ut",
        sources=[str(source)],
        extra_cflags=[
            "-O3",
            "-std=c++20",
...
def test_mtp_future_tokens_are_not_encoded_before_visible(planner_helper):
    ...
    for row, visible_seq_len in enumerate(visible_seq_lens):
        ...
        assert state.physical_rows[ROWS * 2 + row] == (
            row * CAPACITY + state.plan[row, visible_count - 1] - 1
        )
...
def test_missing_current_token_uses_reserved_row_sentinel(planner_helper):
    ...
    assert state.physical_rows[ROWS * 2 : ROWS * 3].tolist() == [
        row * CAPACITY + CAPACITY - 1 for row in range(ROWS)
    ]
