...
def test_read_descriptor_rejects_missing_destination_blocks(monkeypatch):
    monkeypatch.setenv("VLLM_ASCEND_SFA_DEST_BLOCK_WAIT_TIMEOUT_SECONDS", "0")
    thread = _make_read_thread()
...
def test_read_descriptor_waits_for_late_destination_blocks(monkeypatch):
    monkeypatch.setenv("VLLM_ASCEND_SFA_DEST_BLOCK_WAIT_TIMEOUT_SECONDS", "1")
    monkeypatch.setenv("VLLM_ASCEND_SFA_DEST_BLOCK_WAIT_INTERVAL_SECONDS", "0.001")
    thread = _make_read_thread()
    thread._state.dest_blocks_by_req.clear()
