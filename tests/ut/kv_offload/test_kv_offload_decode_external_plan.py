...
def _make_plan_manager():
    ...
    manager.fused_plan_current_linear_slots_npu = (
        manager.fused_plan_metadata_npu[1:]
    )
...
def test_external_lru_planner_thread_warmup_runs_on_tp0():
    ...
    manager.kv_offload_decode_cpp.warmup_lru_resident_threads.assert_called_once_with(8)
...
def test_external_lru_planner_thread_warmup_is_gated(
    ...
    manager.kv_offload_decode_cpp.warmup_lru_resident_threads.assert_not_called()
...
def test_mapped_membership_allocation_initializes_external_plan_control():
    ...
    control = membership[
        :,
        FSA_SELECTION_MEMBERSHIP_CONTROL_OFFSET_INT16_COUNT : FSA_SELECTION_MEMBERSHIP_CONTROL_OFFSET_INT16_COUNT
        + FSA_SELECTION_MEMBERSHIP_CONTROL_INT16_COUNT,
    ]
...
def test_external_lru_plan_is_reused_by_three_skip_layers_and_replanned_at_owner():
    ...
    assert manager.prepare_fused_overlap_external_plan(
        layer_name="layer.0", skip_topk=False, **common_args
    )
    ...
    planner = manager.kv_offload_decode_cpp.lru_resident_compact_with_plan_stable_rows
    ...
    assert manager.prepare_fused_overlap_external_plan(
        layer_name="layer.4", skip_topk=False, **common_args
    )
...
def test_eager_external_plan_copies_inputs_and_preserves_cpp_argument_order():
    ...
    plan_start = FSA_SELECTION_MEMBERSHIP_CONTROL_OFFSET_INT16_COUNT - manager.topk
    planner = manager.kv_offload_decode_cpp.lru_resident_compact_with_plan_stable_rows
